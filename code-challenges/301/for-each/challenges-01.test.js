...
// Idea taken from https://stackoverflow.com/questions/10993824/do-something-n-times-declarative-syntax#

const addNumbers = (num, arr, times, callback) => {
  Array(times).fill(times).forEach(() => {callback(arr, num)});
  return arr;
};
...
const removeOne = (num, arr) => {if (num % 3 === 2) {arr.pop()}};
...
const removeWithForEach = (arr, callback) => {
  arr.forEach((value, i) => callback(arr[i], arr) );
  return arr;
};
